...
Ask clarifying questions only when the missing information matters and cannot be reasonably inferred. Respect a user-specified location; otherwise create user-wide skills in `$HOME/.agents/skills`. Keep `$CODEX_HOME/skills/.system` for Codex-managed system skills.
...
scripts/init_skill.py my-skill --path "$HOME/.agents/skills" --resources references
```
